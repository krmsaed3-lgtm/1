...
// - If missing, call Supabase Edge Function `get_new_address`.
...
;(function () {
  ...
  var EDGE_FUNCTION = (window.DEPOSIT_EDGE_FUNCTION || 'get_new_address');
  ...
  // Your system is "Demo Auth" (not Supabase Auth). We use localStorage user id.
  function getCurrentUserId() {
    try {
      var id = localStorage.getItem('currentUserId') || localStorage.getItem('sb_user_id_v1') || '';
      if (id) return Promise.resolve(String(id));
    } catch (_e) {}

    // Fallback only (in case you later switch to Supabase Auth)
    try {
  ...
  function fetchExistingAddress(userId) {
    ...
    var url = SB.url + '/rest/v1/deposit_addresses'
      + '?select=pay_address,network,pay_currency,updated_at'
  ...
  function requestAddressFromEdge(userId) {
    if (!SB || !SB.url || !SB.headers) return Promise.resolve(null);
    ...
    return fetch(fnUrl, {
      ...
      .then(function (r) {
        if (!r.ok) {
          return safeJson(r).then(function (e) { throw e || new Error('Edge function error'); });
        }
        return safeJson(r);
      })
  ...
  function boot() {
    ...
    getCurrentUserId().then(function (userId) {
      ...
      return fetchExistingAddress(userId).then(function (row) {
        ...
        return requestAddressFromEdge(userId).then(function (data) {
          if (!data) return;
