...
(function () {
  ...
  async function fetchMyLevelState(userId) {
    const base = getSupabaseUrl();
    ...
    return {
      current_level: (row && typeof row.current_level === "string" && row.current_level) ? row.current_level : null,
      is_locked: row ? (row.is_locked === true) : false,
      locked_reason: row ? (typeof row.locked_reason === "string" ? row.locked_reason : null) : null,
    };
  }
  ...
  // Requirements (align with DB for V1..V3)
  const RULES = {
  ...
  const LEVEL_ORDER = ["V0","V1","V2","V3","V4","V5","V6"];
  ...
  function showLockMessage(msg) {
    alert(msg || 'This level is locked. Please contact support.');
  }

  function explainRequirements(level) {
    const r = RULES[level];
    if (!r) return '';
    const parts = [];
    if (r.minBalance > 0) parts.push(`USDT balance >= ${r.minBalance}`);
    if (r.minUsers > 0) parts.push(`Effective users (Gen-1) >= ${r.minUsers}`);
    return parts.length ? parts.join(' and ') : '';
  }

  function attachTabGuards(state) {
    const tabs = document.querySelectorAll('.tabs .tab');
    if (!tabs.length) return;

    const current = state.currentLevel || 'V0';
    const currentIdx = levelIndex(current);

    tabs.forEach(function(tab) {
      tab.addEventListener('click', function(ev) {
        // capture-phase handler blocks the inline handler in member.html
        ev.preventDefault();
        ev.stopImmediatePropagation();

        const target = tab.getAttribute('data-level') || 'V1';
        const targetIdx = levelIndex(target);

        // If account is locked (V3), allow viewing up to V3 only, but show reason.
        if (state.isLocked) {
          if (targetIdx > levelIndex('V3')) {
            showLockMessage(state.lockedReason || 'Reached V3. Manual review required. Please contact support.');
            setActive('V3');
            return;
          }
          setActive(target);
          // show message once when clicking V3
          if (target === 'V3') {
            showLockMessage(state.lockedReason || 'Reached V3. Manual review required. Please contact support.');
          }
          return;
        ...
        // Not locked: prevent navigating to higher-than-current levels (these are locked).
        if (targetIdx > currentIdx) {
          const req = explainRequirements(target);
          showLockMessage(req ? (`Locked. Requirements: ${req}.`) : 'Locked.');
          setActive(currentIdx > 0 ? LEVEL_ORDER[currentIdx] : 'V1');
          return;
        }
  ...
  async function loadMemberData() {
    ...
    const [balance, levelState, team] = await Promise.all([
      fetchMyDemoBalance(userId),
      fetchMyLevelState(userId),
      rpcGetMyTeam(userId),
    ]);
    ...
    // Update top bar
    const lvl = (levelState && levelState.current_level) ? levelState.current_level : "V0";
    const next = nextLevelFromCurrent(lvl);
    updateTopBar(lvl, next);

    // Guard tab switching based on DB-driven current_level / is_locked
    attachTabGuards({
      currentLevel: lvl,
      isLocked: !!(levelState && levelState.is_locked),
      lockedReason: levelState ? levelState.locked_reason : null,
    });
