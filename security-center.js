(function () {
  ...
  if (emSubmit) {
    emSubmit.addEventListener('click', async function () {
      ...
      var codeVal = (emCode && emCode.value) ? emCode.value.trim() : '';
      var passVal = (emPassword && emPassword.value) ? emPassword.value.trim() : '';

      resetEmailErrors();

      // Basic validation for password length (already enforced by updateEmailSubmitState)
      if (!passVal || passVal.length < 8) {
        var ep = document.getElementById('em-password-error');
        if (ep) ep.textContent = 'Login password must be at least 8 characters.';
        return;
      }

      emSubmit.disabled = true;
      try {
        // Verify the login password via RPC before confirming email code.
        // If this function isn't defined in your DB, it will throw and we treat as failure.
        var ok = await rpc('check_login_password', { p_user: userId, p_login: passVal });
        if (!(ok === true || ok === 't')) {
        ...
        // Now confirm the email code via Edge function
        await callEdge('confirm_email_code', {
          user_id: userId,
          email: (emEmail ? String(emEmail.value || '').trim() : ''),
          code: codeVal
        });
