...
> yazan:
const funded1 = gen1.filter(r => r.is_funded === true).length;
    const funded2 = gen2.filter(r => r.is_funded === true).length;
    const funded3 = gen3.filter(r => r.is_funded === true).length;
  ...
  function renderGen1Table(rows) {
    ...
    for (const r of rows) {
      const lvl = (r.is_funded === true) ? "LV.1" : "LV.0";
      const tr = document.createElement("tr");
      tr.innerHTML = 
